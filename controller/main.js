function character() {
  ...
  if (queryParameterName !== null && queryParameterName !== "") {
    ...
    document.getElementById("characterSection").innerHTML =
      '<h2 id="characterMainTitle">DIGITE A PESQUISA...</h2>';
  }
}
...
function connection() {
  ...
  xhr.onerror = function() {
    document.getElementById("characterSection").innerHTML = '<h2 id="characterMainTitle">Erro de conexão da Marvel API</h2>';
  }
  xhr.onload = function() {
    if (this.status == 200) {
      ...
      if (results["data"].count === 0) {
        ...
        document.getElementById("characterSection").innerHTML =
          ...
          "Desculpe, servidor Marvel sem resposta<br>tente mais tarde.</h2>";
      ...
      document.getElementById("characterSection").innerHTML = '<h2 id="characterMainTitle">Falha Request 200</h2>';
    }
  }
...
function comics(characterID) {
  ...
  xhr.onloadstart = function () {
    document.getElementById("comicsSpinnerSection").innerHTML =
      '<strong id="spinnerText" class="text-danger">Carregando histórias...</strong>' +
  ...
  xhr.onload = function () {
    if (this.status == 200) {
      ...
      if (results["data"] == 0) {
        ...
        comicSection.innerHTML = "<h2>Não há quadrinhos disponíveis</h2>";
      } else {
        // comics available
        let output = "",
          creators = "";
          ...
          output +=
          '<h2 id="comicMainTitle">Histórias</h2>' + '<div class="card-columns">';
        ...
        for (const i in comics) {
          if (comics.hasOwnProperty(i)) {
            ...
            output +=
              '<p style="font-size: 12px;" class="card-text text-muted">Personagens: ';
            ...
            output += "</p>";
            output +=
              '<p style="font-size: 12px;" class="card-text text-muted">Criadores: ';
      ...
      document.getElementById("characterSection").innerHTML =
        '<h2 id="characterMainTitle">Falha Request 200</h2>';
      document.getElementById("comicSection").innerHTML =
        '<h2 id="characterMainTitle">Falha Request 200</h2>';
    }
  }
...
function singleComic() {
  ...
  xhr.onloadstart = function () {
    document.getElementById("comicsSpinnerSection").innerHTML =
      '<strong id="spinnerText" class="text-secondary">Carregando quadrinho...</strong>' +
...
function comicCreator() {
  const urlQueryParameters = new URLSearchParams(window.location.search), //http://localhost:8080/creator.php?creator-id=8635&name=luissilva
  ...
  xhr.onload = function () {
    if (this.status == 200) {
      ...
      output +=
        '<h1 class="header-main-title single-comic__main-title">Quadrinhos</h1>' +
